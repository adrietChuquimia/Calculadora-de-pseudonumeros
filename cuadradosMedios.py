...
class GeneradorApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Generador Pseudoaleatorios - Una Semilla (Cuadrados Medios)")
        ...
        tk.Label(frame, text="Semilla (min. 4 dígitos):", bg="#f5f5f5").grid(row=1, column=0, sticky="w")
        self.semilla_var = tk.StringVar()
        tk.Entry(frame, textvariable=self.semilla_var, width=15).grid(row=1, column=1, padx=5)

        tk.Label(frame, text="Nivel de confianza (ej. 0.05):", bg="#f5f5f5").grid(row=2, column=0, sticky="w")
        self.alpha_var = tk.StringVar(value="0.05")
        tk.Entry(frame, textvariable=self.alpha_var, width=10).grid(row=2, column=1, padx=5)
        ...
        self.tree = ttk.Treeview(root, columns=("Iter", "Cuadrado", "Centro", "Número"), show="headings", height=15)
        self.tree.heading("Iter", text="Iteración")
        self.tree.heading("Cuadrado", text="Cuadrado")
    ...
    def generar(self):
        try:
            ...
            semilla_str = self.semilla_var.get()

            if not (semilla_str.isdigit() and len(semilla_str) >= 4):
                messagebox.showerror("Error", "La semilla debe ser numérica y tener al menos 4 dígitos.")
                return

            semilla = int(semilla_str)
            n = max(4, len(str(semilla)))
            self.numeros = []
            self.iteraciones = []
            ...
            for i in range(cantidad):
                cuadrado = semilla ** 2
                cuad_str = str(cuadrado)

                if len(cuad_str) % 2 != 0:
                    cuad_str = "0" + cuad_str

                inicio = (len(cuad_str) - n) // 2
                centro = cuad_str[inicio: inicio + n]

                numero_final = int(centro) / (10 ** n)
                self.numeros.append(numero_final)
                self.iteraciones.append((i+1, cuad_str, centro, numero_final))

                self.tree.insert("", "end", values=(i+1, cuad_str, centro, f"{numero_final:.4f}"))
        ...
        if archivo.endswith(".txt"):
            with open(archivo, "w") as f:
                for it in self.iteraciones:
                    f.write(f"Iter {it[0]}: cuadrado={it[1]}, centro={it[2]}, decimal={it[3]}\n")

        elif archivo.endswith(".xlsx"):
            df = pd.DataFrame(self.iteraciones, columns=["Iteración", "Cuadrado", "Centro", "Número"])
            df.to_excel(archivo, index=False)

        elif archivo.endswith(".rtf"):
            contenido = "\n".join([f"Iter {it[0]}: cuadrado={it[1]}, centro={it[2]}, decimal={it[3]}" for it in self.iteraciones])
